...
2D 引擎在架构上与 3D 引擎共享核心设计——游戏循环、组件系统、渲染管线、输入管理、音频系统等，但绕开了 3D 引擎特有的复杂度（透视投影、矩阵变换、光照模型、深度测试等），让我能更快触及引擎架构的本质。
...
3. 纹理加载与精灵渲染
4. 数学库（向量、矩阵、变换）
5. 资源管理器（纹理、音频、Shader）
6. 输入系统
7. 音频系统
8. 场景图系统（Node 树、SceneGraph、变换层级）
9. UI 编辑器（类似 Cocos Studio / UE UMG）
10. 2D 物理（AABB、碰撞检测）
11. 粒子系统
12. 性能分析与调试工具
