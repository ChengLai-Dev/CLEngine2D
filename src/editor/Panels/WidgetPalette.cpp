...
void WidgetPalette::OnRender(Renderer& renderer) {
    ...
    renderer.DrawQuad(Vec3(m_rectWidth * 0.5f, renderHeight * 0.5f, 0.0f),
                      Vec3(m_rectWidth, renderHeight, 1.0f), bgColor);

    static const char* btnLabels[] = { "Button", "Label", "Image", "Panel", "Layout" };

    for (int i = 0; i < kBtnCount; ++i) {
        HitRect r = GetButtonRect(i);
        renderer.DrawQuad(Vec3(r.x + r.w * 0.5f, r.y + r.h * 0.5f, 0.0f),
                          Vec3(r.w - 2.0f, r.h, 1.0f), btnColor);
